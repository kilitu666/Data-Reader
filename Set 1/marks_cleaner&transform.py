...
def mark_str_to_dict_revised(mark_str: str) -> Dict[str, int | float]:
    ...
    for grade in items:  # define each grade, e.g., "A1: 99"
        ...
        num = float(value.strip())  # parse value to float
        fixed = fix_invalid_value(num) # fix invalid values
        is_finite = fixed != float('inf') and fixed != float('-inf') # check the value is finite
...
def summarize_marks(marks: Dict[str, Dict], split: str) -> dict:
    ...
    for record in marks.values():   # iterate through each student's marks
        if not isinstance(record, dict): # check if record is a dictionary
            invalid_count += 1
            continue

        if split not in record: # check if the subject exists
            invalid_count += 1
            continue

        value = record[split]
        is_finite = value != float('inf') and value != float('-inf') # check if the value is finite
    ...
    if valid_count == 0: # no valid marks
        average = float('-inf')  
    else:
        avg = total / valid_count
        average = int(avg) if avg == int(avg) else avg # convert to int if whole number
